...
import {uploadnews , getnews, updateNewsAnalysisId, getNewsAnalysisId } from './supabase/supabase_functions.js';
...
async function fetchNews() {
      ...
      try {
        ...
        if (response.status === 200) {
          ...
          $('item').each((index, element) => {
            ...
            const pubDateFormatted = moment(pubdate, 'ddd, DD MMM YYYY HH:mm:ss Z').format('YYYY-MM-DD');
  ...
  async function saveDataToSupabase(newsData) {
    ...
    if (existingNews.error) {
      console.error('Error al obtener las noticias de la base de datos:', existingNews.error.message);
      return;
    }
    ...
    const newsToUpload = newNews.filter(news => !existingNews.data.find(existingNews => existingNews.title === news.title));
    ...
    for (const news of newsToUpload) {
      ...
      if (error) {
        console.error('Error al subir noticias a la base de datos:', error.message);
        return;
      }
