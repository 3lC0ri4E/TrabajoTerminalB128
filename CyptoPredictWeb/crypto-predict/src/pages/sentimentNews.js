...
async function getCurrentSentiment(newsData) {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          if (errorData.error && errorData.error.includes('currently loading')) {
            console.log(`Modelo cargando, reintentando en 5 segundos... (Intento ${attempt + 1})`);
            await sleep(5000); // Espera 5 segundos antes de reintentar
            continue;
          } else {
            throw new Error(`Error en la API de Hugging Face: ${response.statusText} - ${JSON.stringify(errorData)}`);
          }
        }
  ...
  const { error: insertError } = await insertSentiment(currentDate, generalSentiment, percentage);
  if (insertError) {
    console.error('Error al guardar el sentimiento en la base de datos:', insertError.message);
  } else {
